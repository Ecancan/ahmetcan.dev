...
const Page: FC<PageProps> = (props) => {
    ...
    const cards = [
        ...
        },
        {
            title: 'Learn',
            link: '/learn',
            description: 'The page where I diligently write what I\'ve learned. If there\'s nothing, it means I\'m currently feeling lazy.',
        },
        {
            title: 'Bookmarks',
            link: '/bookmarks',
        ...
        <BaseView className={'justify-between h-full'}>
            ...
            <BaseView className={'flex-1 mb-4 items-center justify-center'}>
                <BaseView className={'flex-row items-center gap-8'}>
                    <GlowingImage src={'/assets/img/profile.jpeg'} width={150} height={150} alt={'profile'} />
                    <GlowingText text={'.dev'} />
                </BaseView>
            </BaseView>
            <BaseView className={'flex-row gap-4 mb-12'}>
                {cards.map((card, index) => <LinkCard key={index} {...card} />)}
            </BaseView>
        </BaseView>
    );
};
