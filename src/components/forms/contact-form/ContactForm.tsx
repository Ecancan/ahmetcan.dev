...
const ContactForm = () => (
  <BaseView className={'gap-8'}>
    <BaseInput
      name={'topic'}
      label={'Topic'}
      placeholder={'You can select topic name for getting payload.'}
    />
    <BaseInput
      name={'origin'}
      label={'Origin'}
      placeholder={'You have to define Origin like email, name-surname etc.'}
    />
    <BaseTextarea
      name={'payload'}
      label={'Payload'}
      placeholder={'You can send any payload with this input.'}
      rows={15}
    />
  </BaseView>
);
